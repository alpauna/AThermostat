...
#   ./scripts/ota-update.sh --revert  — revert to previous firmware backup on LittleFS

set -e

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
BUILD_DIR="$SCRIPT_DIR/../.pio/build/esp32s3devkitc1"
...
if [ ! -f "$FIRMWARE" ]; then
    ...
    echo "Run 'pio run -e esp32s3devkitc1' first."
    exit 1
fi
...
# Step 1: Upload to LittleFS
echo
echo "Step 1/3: Uploading firmware to flash storage..."
...
echo "Firmware uploaded to flash storage."
...
if [ -z "$APPLY_STATUS" ]; then
    ...
    if [ -z "$UPLOADED_EXISTS" ]; then
        echo "Error: Firmware not found on flash after upload"
        exit 1
    fi
    UPLOADED_SIZE=$(echo "$APPLY_STATUS" | grep -o '"size":[0-9]*' | cut -d: -f2)
    if [ "$UPLOADED_SIZE" != "$FW_SIZE" ]; then
        echo "Error: Size mismatch — local ${FW_SIZE} bytes, flash ${UPLOADED_SIZE} bytes"
        exit 1
    fi
    echo "Verified: ${FW_SIZE_KB} KB on flash matches local firmware."
fi
...
NEW_HEAP=$(curl $CURL_OPTS --connect-timeout 5 $AUTH_OPTS "$BASE_URL/heap" 2>/dev/null) || true
if [ -n "$NEW_HEAP" ]; then
...
if [ -n "$PENDING" ]; then
    echo "Warning: /firmware.new still on flash (apply may not have completed)"
fi
if [ -n "$BACKUP" ]; then
